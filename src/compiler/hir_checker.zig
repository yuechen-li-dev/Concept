...
const synthetic_span = diagnostics.SourceSpan{ .start = 0, .length = 0 };
...
const Checker = struct {
    ...
    fn checkModule(self: *Checker) CheckError!void {
        ...
        if (!sameType(main.return_type, self.module.types.intType())) {
            try self.report(.InvalidMainSignature, "main must return int");
            return error.InvalidSemanticModule;
        }
        if (main.params.len != 0) {
            try self.report(.InvalidMainSignature, "main must not have parameters");
            return error.InvalidSemanticModule;
        }
        if (main.body == null) {
            try self.report(.InvalidMainSignature, "main must have a body");
            return error.InvalidSemanticModule;
        }
        ...
        switch (stmt) {
            ...
            .return_stmt => |maybe_value| {
                const value = maybe_value orelse {
                    try self.report(.TypeMismatch, "return statements must have an expression");
                    return error.InvalidSemanticModule;
                };
                const value_type = try self.checkExpr(value);
                try self.requireSame(value_type, return_type, "return expression type does not match function return type");
            },
            .local_decl => |decl| {
                const local = self.module.hir.getLocal(decl.local);
                const init_type = try self.checkExpr(decl.initializer);
                try self.requireSame(init_type, local.type_id, "local initializer type does not match declared type");
            },
            .assignment => |assignment| {
                ...
                try self.requireSame(value_type, target_type, "assignment expression type does not match target type");
            },
            .if_stmt => |if_stmt| {
                const condition_type = try self.checkExpr(if_stmt.condition);
                try self.requireBool(condition_type, "if condition must be bool");
            ...
            .while_stmt => |while_stmt| {
                ...
                try self.requireBool(condition_type, "while condition must be bool");
            ...
            .match_stmt => |match_stmt| {
                ...
                if (!self.isInt(scrutinee_type) and !self.isBool(scrutinee_type)) {
                    try self.report(.TypeMismatch, "match scrutinee must be int or bool");
                    return error.InvalidSemanticModule;
                }
                ...
                for (match_stmt.arms) |arm| {
                    switch (arm.pattern) {
                        .int_literal => |text| {
                            try self.requireSame(self.module.types.intType(), scrutinee_type, "match pattern type does not match scrutinee type");
                            if (seen_ints.contains(text)) {
                                try self.report(.TypeMismatch, "duplicate literal match pattern");
                                return error.InvalidSemanticModule;
                            }
                            try seen_ints.put(text, {});
                        },
                        .bool_literal => |value| {
                            try self.requireSame(self.module.types.boolType(), scrutinee_type, "match pattern type does not match scrutinee type");
                        ...
                        .wildcard => {
                            if (seen_wildcard) {
                                try self.report(.TypeMismatch, "duplicate wildcard match pattern");
                                return error.InvalidSemanticModule;
                            }
                            seen_wildcard = true;
                        },
    ...
    fn checkExpr(self: *Checker, expr_id: hir.ExprId) CheckError!types.TypeId {
        ...
        return switch (expr) {
            ...
            .call => |call| blk: {
                ...
                if (callee.body == null) {
                    try self.report(.InvalidCall, "cannot call function without body");
                    return error.InvalidSemanticModule;
                }
                if (call.args.len != callee.params.len) {
                    try self.report(.InvalidCall, "function call argument count mismatch");
                    return error.InvalidSemanticModule;
                }
                for (call.args, callee.params) |arg, param_id| {
                    ...
                    try self.requireCallSame(arg_type, param_type, "function call argument type mismatch");
                }
            ...
            .unary => |unary| blk: {
                ...
                switch (unary.op) {
                    .negate => {
                        try self.requireInt(operand_type, "arithmetic unary operator requires int operand");
                        break :blk self.module.types.intType();
                    },
                    .logical_not => {
                        try self.requireBool(operand_type, "logical unary operator requires bool operand");
            ...
            .binary => |binary| blk: {
                ...
                switch (binary.op) {
                    .add, .subtract, .multiply, .divide, .modulo => {
                        try self.requireIntPair(left_type, right_type, "arithmetic binary operator requires int operands");
                        break :blk self.module.types.intType();
                    },
                    .less, .less_equal, .greater, .greater_equal => {
                        try self.requireIntPair(left_type, right_type, "comparison operator requires int operands");
                        break :blk self.module.types.boolType();
                    },
                    .equal_equal, .bang_equal => {
                        if (!sameType(left_type, right_type) or (!self.isInt(left_type) and !self.isBool(left_type))) {
                            try self.report(.TypeMismatch, "equality operator requires matching int or bool operands");
                            return error.InvalidSemanticModule;
                        }
                        break :blk self.module.types.boolType();
                    },
                    .logical_and, .logical_or => {
                        try self.requireBoolPair(left_type, right_type, "logical binary operator requires bool operands");
                        break :blk self.module.types.boolType();
                    },
                }
            },
    ...
    fn requireSame(self: *Checker, actual: types.TypeId, expected: types.TypeId, message: []const u8) CheckError!void {
        if (!sameType(actual, expected)) {
            try self.report(.TypeMismatch, message);
            return error.InvalidSemanticModule;
        }
    }

    fn requireCallSame(self: *Checker, actual: types.TypeId, expected: types.TypeId, message: []const u8) CheckError!void {
        if (!sameType(actual, expected)) {
            try self.report(.InvalidCall, message);
            return error.InvalidSemanticModule;
        }
    }

    fn requireInt(self: *Checker, actual: types.TypeId, message: []const u8) CheckError!void {
    ...
    fn requireBool(self: *Checker, actual: types.TypeId, message: []const u8) CheckError!void {
        if (!self.isBool(actual)) {
            try self.report(.InvalidConditionType, message);
            return error.InvalidSemanticModule;
        }
    }

    fn requireIntPair(self: *Checker, left: types.TypeId, right: types.TypeId, message: []const u8) CheckError!void {
    ...
    fn requireBoolPair(self: *Checker, left: types.TypeId, right: types.TypeId, message: []const u8) CheckError!void {
    ...
    fn report(self: *Checker, code: diagnostics.DiagnosticCode, message: []const u8) !void {
        if (self.diagnostics) |bag| try bag.append(diagnostics.makeDiagnostic(code, .@"error", message, synthetic_span));
    }
};
...
const TestModule = struct {
    ...
    fn int(self: *TestModule, text: []const u8) !hir.ExprId {
        return self.module.hir.addExpr(.{ .int_literal = try std.testing.allocator.dupe(u8, text) });
    }

    fn boolLit(self: *TestModule, value: bool) !hir.ExprId {
        return self.module.hir.addExpr(.{ .bool_literal = value });
    }

    fn ret(self: *TestModule, expr: ?hir.ExprId) !hir.StmtId {
        return self.module.hir.addStmt(.{ .return_stmt = expr });
    }
...
test "HIR checker accepts locals and assignments" {
    ...
    const init_x = try tm.module.hir.addStmt(.{ .local_decl = .{ .local = x, .initializer = try tm.int("1") } });
    const init_b = try tm.module.hir.addStmt(.{ .local_decl = .{ .local = b, .initializer = try tm.boolLit(true) } });
    const add = try tm.module.hir.addExpr(.{ .binary = .{ .op = .add, .left = try tm.module.hir.addExpr(.{ .local_ref = x }), .right = try tm.int("2") } });
    const assign_x = try tm.module.hir.addStmt(.{ .assignment = .{ .target = .{ .local = x }, .value = add } });
    const ret_stmt = try tm.ret(try tm.module.hir.addExpr(.{ .local_ref = x }));
...
test "HIR checker accepts function calls" {
    ...
    const sum = try tm.module.hir.addExpr(.{ .binary = .{ .op = .add, .left = try tm.module.hir.addExpr(.{ .param_ref = a }), .right = try tm.module.hir.addExpr(.{ .param_ref = b }) } });
    ...
    tm.setBody(main_id, try tm.block(&.{try tm.ret(try tm.module.hir.addExpr(.{ .call = .{ .function = add_id, .args = args } }))}));
    try tm.checkPass();
}
...
test "HIR checker accepts if match while" {
    ...
    const main_id = try tm.function("main", tm.module.types.intType());
    const x = try tm.local(main_id, "x", tm.module.types.intType());
    const init_x = try tm.module.hir.addStmt(.{ .local_decl = .{ .local = x, .initializer = try tm.int("0") } });
    const while_body = try tm.block(&.{try tm.module.hir.addStmt(.{ .assignment = .{ .target = .{ .local = x }, .value = try tm.int("1") } })});
    const while_stmt = try tm.module.hir.addStmt(.{ .while_stmt = .{ .condition = try tm.boolLit(false), .body = while_body } });
    const match_body = try tm.module.hir.addStmt(.{ .assignment = .{ .target = .{ .local = x }, .value = try tm.int("2") } });
    const arms = try std.testing.allocator.dupe(hir.HirMatchArm, &.{.{ .pattern = .{ .int_literal = try std.testing.allocator.dupe(u8, "1") }, .body = match_body }});
    const match_stmt = try tm.module.hir.addStmt(.{ .match_stmt = .{ .scrutinee = try tm.module.hir.addExpr(.{ .local_ref = x }), .arms = arms } });
    const then_block = try tm.block(&.{match_stmt});
    const if_stmt = try tm.module.hir.addStmt(.{ .if_stmt = .{ .condition = try tm.boolLit(true), .then_block = then_block, .else_block = null } });
    const ret_stmt = try tm.ret(try tm.module.hir.addExpr(.{ .local_ref = x }));
...
test "HIR checker rejects return local and assignment mismatches" {
    ...
    local_tm.setBody(local_main, try local_tm.block(&.{try local_tm.module.hir.addStmt(.{ .local_decl = .{ .local = x, .initializer = try local_tm.boolLit(false) } })}));
    ...
    const decl_y = try assign_tm.module.hir.addStmt(.{ .local_decl = .{ .local = y, .initializer = try assign_tm.int("0") } });
    const assign_y = try assign_tm.module.hir.addStmt(.{ .assignment = .{ .target = .{ .local = y }, .value = try assign_tm.boolLit(true) } });
...
test "HIR checker rejects bad conditions calls and match patterns" {
    ...
    if_tm.setBody(if_main, try if_tm.block(&.{try if_tm.module.hir.addStmt(.{ .if_stmt = .{ .condition = try if_tm.int("1"), .then_block = try if_tm.block(&.{}), .else_block = null } })}));
    ...
    while_tm.setBody(while_main, try while_tm.block(&.{try while_tm.module.hir.addStmt(.{ .while_stmt = .{ .condition = try while_tm.int("1"), .body = try while_tm.block(&.{}) } })}));
    ...
    call_count.setBody(call_main, try call_count.block(&.{try call_count.ret(try call_count.module.hir.addExpr(.{ .call = .{ .function = callee, .args = no_args } }))}));
    ...
    call_type.setBody(main, try call_type.block(&.{try call_type.ret(try call_type.module.hir.addExpr(.{ .call = .{ .function = id, .args = bad_args } }))}));
    ...
    const arms = try std.testing.allocator.dupe(hir.HirMatchArm, &.{.{ .pattern = .{ .bool_literal = true }, .body = body }});
    match_tm.setBody(match_main, try match_tm.block(&.{try match_tm.module.hir.addStmt(.{ .match_stmt = .{ .scrutinee = try match_tm.int("0"), .arms = arms } })}));
...
test "HIR checker rejects duplicate match pattern and wildcard" {
    ...
    const arms = try std.testing.allocator.dupe(hir.HirMatchArm, &.{ .{ .pattern = .{ .int_literal = try std.testing.allocator.dupe(u8, "1") }, .body = body }, .{ .pattern = .{ .int_literal = try std.testing.allocator.dupe(u8, "1") }, .body = body } });
    dup_lit.setBody(lit_main, try dup_lit.block(&.{try dup_lit.module.hir.addStmt(.{ .match_stmt = .{ .scrutinee = try dup_lit.int("0"), .arms = arms } })}));
    ...
    const wild_arms = try std.testing.allocator.dupe(hir.HirMatchArm, &.{ .{ .pattern = .wildcard, .body = wild_body }, .{ .pattern = .wildcard, .body = wild_body } });
    dup_wild.setBody(wild_main, try dup_wild.block(&.{try dup_wild.module.hir.addStmt(.{ .match_stmt = .{ .scrutinee = try dup_wild.boolLit(false), .arms = wild_arms } })}));
...
test "HIR checker accepts Phase 2-style lowered program" {
    ...
    const main_id = try tm.function("main", tm.module.types.intType());
    const x = try tm.local(main_id, "x", tm.module.types.intType());
    const decl = try tm.module.hir.addStmt(.{ .local_decl = .{ .local = x, .initializer = try tm.int("4") } });
    const then_body = try tm.block(&.{try tm.ret(try tm.int("9"))});
    const arms = try std.testing.allocator.dupe(hir.HirMatchArm, &.{ .{ .pattern = .{ .int_literal = try std.testing.allocator.dupe(u8, "4") }, .body = then_body }, .{ .pattern = .wildcard, .body = try tm.block(&.{try tm.ret(try tm.int("1"))}) } });
    const match_stmt = try tm.module.hir.addStmt(.{ .match_stmt = .{ .scrutinee = try tm.module.hir.addExpr(.{ .local_ref = x }), .arms = arms } });
    const condition = try tm.module.hir.addExpr(.{ .binary = .{ .op = .greater_equal, .left = try tm.module.hir.addExpr(.{ .local_ref = x }), .right = try tm.int("4") } });
    const if_stmt = try tm.module.hir.addStmt(.{ .if_stmt = .{ .condition = condition, .then_block = try tm.block(&.{match_stmt}), .else_block = try tm.block(&.{try tm.ret(try tm.int("0"))}) } });
