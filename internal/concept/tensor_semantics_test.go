...
var tensorConformanceCases = []ConformanceCase{
...
func TestTensorSemanticsConformance(t *testing.T) {
	counts := map[ConformanceExpectation]int{}
	for _, tc := range tensorConformanceCases {
		t.Run(tc.Name, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1", "tensor", "semantics", filepath.FromSlash(tc.Source))
			source, err := os.ReadFile(path)
			if err != nil {
				t.Fatal(err)
			}
...
func TestTensorSemanticsMIRValidationRejectsIncompleteTensor(t *testing.T) {
...
func TestTensorSemanticsRuntimeGuards(t *testing.T) {
