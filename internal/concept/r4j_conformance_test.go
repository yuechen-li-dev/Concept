...
var evt1R4jConformanceCases = []ConformanceCase{
...
func generateR4jFixture(t *testing.T, class, file string) Outputs {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1-r4j", "core", class, file)
...
func r4jMIR(t *testing.T, file string) (MIR, Outputs) {
	t.Helper()
...
func TestEVT1R4jConformance(t *testing.T) {
	counts := map[ConformanceExpectation]int{}
	for _, tc := range evt1R4jConformanceCases {
		t.Run(tc.Name, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1-r4j", "core", filepath.FromSlash(tc.Source))
			source, err := os.ReadFile(path)
			if err != nil {
				t.Fatal(err)
			}
...
func TestEVT1R4jProofsAreTypedAndDeterministic(t *testing.T) {
...
func TestEVT1R4jLayoutStreamSpanTensorPreservation(t *testing.T) {
	mir, outputs := r4jMIR(t, "fact_preservation_chain.concept")
...
func TestEVT1R4jAlignmentDegradesAndIntervalsProveDisjoint(t *testing.T) {
	mir, _ := r4jMIR(t, "fact_alignment_subspan.concept")
	degraded := false
	for _, fact := range mir.SemanticFacts {
	...
	disjointMIR, _ := r4jMIR(t, "fact_disjoint_subspans.concept")
	found := false
...
func TestEVT1R4jTensorBackingQualification(t *testing.T) {
	fixed, _ := r4jMIR(t, "fact_disjoint_inline_tensors.concept")
	...
	runtime, _ := r4jMIR(t, "fact_runtime_shape_bound_tensor.concept")
	runtimeShape := false
...
func TestEVT1R4jMIRRejectsIncompleteFact(t *testing.T) {
