...
var evt1R5jInvalidCases = map[string]string{
...
func r5jFixture(t *testing.T, kind, name string) (Module, []byte) {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1-r5j", "core", kind, name)
...
func TestEVT1R5jCorpusConformance(t *testing.T) {
	valid, _ := filepath.Glob(filepath.Join("..", "..", "language", "evt1-r5j", "core", "valid", "*.concept"))
	invalid, _ := filepath.Glob(filepath.Join("..", "..", "language", "evt1-r5j", "core", "invalid", "*.concept"))
	...
	for _, path := range invalid {
		t.Run("invalid/"+filepath.Base(path), func(t *testing.T) {
			...
			if got, want := diagnosticCode(err), evt1R5jInvalidCases[filepath.Base(path)]; got != want {
...
func TestEVT1R5jCanonicalUsingAndTypeCompatibility(t *testing.T) {
...
func TestEVT1R5jMIRPlanAndMachinePersistence(t *testing.T) {
	module, source := r5jFixture(t, "valid", "callable_machine_field_ref_capture.concept")
...
func TestEVT1R5jOwnedMachineCallableHasOneGuardedCleanupPath(t *testing.T) {
	module, source := r5jFixture(t, "valid", "callable_machine_field_owned_capture.concept")
	outputs, err := Generate(module, source)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEVT1R5jDeterminismAndForbiddenRuntime(t *testing.T) {
	module, source := r5jFixture(t, "valid", "callable_machine_field_ref_capture.concept")
...
func TestEVT1R5jNativeC11(t *testing.T) {
	...
	for name, want := range cases {
		t.Run(name, func(t *testing.T) {
			module, source := r5jFixture(t, "valid", name+".concept")
			outputs, err := Generate(module, source)
			if err != nil {
				t.Fatal(err)
			}
			harness := fmt.Sprintf("#include \"%s.generated.h\"\nint main(void) { return concept_%s_main() == %d ? 0 : 1; }\n", name, name, want)
			runR1NativeHarness(t, outputs, "r5j_"+name+"_harness.c", harness)
		})
	}
}
