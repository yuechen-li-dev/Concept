...
var evt1R5aInvalid = map[string]string{
...
func r5aFixture(t *testing.T, class, file string) Outputs {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1-r5a", "core", class, file)
...
func TestEVT1R5aConformance(t *testing.T) {
	for _, file := range evt1R5aValid {
		t.Run("valid/"+file, func(t *testing.T) { r5aFixture(t, "valid", file) })
	}
	for file, code := range evt1R5aInvalid {
		t.Run("invalid/"+file, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1-r5a", "core", "invalid", file)
			source, err := os.ReadFile(path)
			if err != nil {
				t.Fatal(err)
			}
...
func TestEVT1R5aMIRAndPlanner(t *testing.T) {
	outputs := r5aFixture(t, "valid", "automata_basic.concept")
	var mir MIR
	...
	modulePath := filepath.Join("..", "..", "language", "evt1-r5a", "core", "valid", "automata_basic.concept")
...
func TestEVT1R5aNativeC11(t *testing.T) {
	cases := []struct {
		file, call string
		want       int
	}{
	...
	for _, tc := range cases {
		t.Run(tc.file, func(t *testing.T) {
			outputs := r5aFixture(t, "valid", tc.file)
			...
			runR1NativeHarness(t, outputs, "r5a_harness.c", harness)
...
func TestEVT1R5aCleanupShape(t *testing.T) {
	owned := string(r5aFixture(t, "valid", "automata_with_state_owned.concept")["automata_with_state_owned.generated.c"])
	...
	transient := string(r5aFixture(t, "valid", "automata_with_state_owned.concept")["automata_with_state_owned.generated.c"])
	if !strings.Contains(transient, "concept_owner_drop(&owner);") {
		t.Fatal("automata lifetime cleanup missing")
	}
	shape := string(r5aFixture(t, "valid", "automata_basic.concept")["automata_basic.generated.c"])
	...
	transientOwned := string(r5aFixture(t, "valid", "machine_transient_local.concept")["machine_transient_local.generated.c"])
