...
func asyncControlFlowFixture(t *testing.T, kind, name string) (Module, []byte) {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1", "async", "control-flow", kind, name)
...
func TestAsyncControlFlowCorpusAndComplexAliases(t *testing.T) {
	valid, err := filepath.Glob(filepath.Join("..", "..", "language", "evt1", "async", "control-flow", "valid", "*.concept"))
	if err != nil {
		t.Fatal(err)
	}
	invalid, err := filepath.Glob(filepath.Join("..", "..", "language", "evt1", "async", "control-flow", "invalid", "*.concept"))
	if err != nil {
		t.Fatal(err)
	}
	...
	readAsync := func(name string) *MIRAsyncFunction {
		module, source := asyncControlFlowFixture(t, "valid", name)
...
func TestAsyncControlFlowStructuredNativeC11(t *testing.T) {
	for _, name := range []string{
		...
		t.Run(name, func(t *testing.T) {
			module, source := asyncControlFlowFixture(t, "valid", name+".concept")
			outputs, err := Generate(module, source)
			if err != nil {
...
func TestAsyncControlFlowGraphPlanAndDeterminism(t *testing.T) {
	module, source := asyncControlFlowFixture(t, "valid", "async_match_multiple_awaits.concept")
	outputs, err := Generate(module, source)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAsyncControlFlowStaticDiagnostics(t *testing.T) {
	...
	for name, want := range cases {
		path := filepath.Join("..", "..", "language", "evt1", "async", "control-flow", "invalid", name)
		source, err := os.ReadFile(path)
		if err != nil {
			t.Fatal(err)
		}
