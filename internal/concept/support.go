...
func (d Diagnostic) SemanticCategory() string {
	switch d.Code {
	case "CONCEPT_ASSERT_REASON_REQUIRED", "CONCEPT_ASSERT_REASON_NONEMPTY", "CONCEPT_ASSERT_SUBJECT_NOT_QUERYABLE", "CONCEPT_ASSERT_SUBJECT_UNKNOWN", "CONCEPT_ASSERT_SUBJECT_AMBIGUOUS", "CONCEPT_ASSERT_GOAL_INVALID", "CONCEPT_ASSERT_GOAL_UNKNOWN", "CONCEPT_ASSERT_PARAMETER_INVALID", "CONCEPT_ASSERT_ARITY_MISMATCH", "CONCEPT_ASSERT_DISPROVEN", "CONCEPT_ASSERT_UNKNOWN":
		return d.Code
	case "TEST_ATTRIBUTE_INVALID", "TEST_ATTRIBUTE_REQUIRES_FUNCTION", "TEST_ATTRIBUTE_ARGUMENT_INVALID", "TEST_ATTRIBUTE_UNKNOWN", "TEST_KIND_REQUIRED", "TEST_KIND_CONFLICT", "TEST_DUPLICATE_IDENTITY", "TEST_FACT_REQUIRES_VOID_RETURN", "TEST_FACT_REQUIRES_ZERO_PARAMETERS", "TEST_THEORY_REQUIRES_VOID_RETURN", "TEST_THEORY_REQUIRES_PARAMETERS", "TEST_THEORY_REQUIRES_ARTIFACT", "TEST_THEORY_PARAMETER_UNSUPPORTED", "TEST_THEORY_ARTIFACT_FORMAT", "TEST_THEORY_ARTIFACT_INVALID", "TEST_THEORY_ROW_ARITY", "TEST_THEORY_ROW_TYPE", "TEST_BENCHMARK_REQUIRES_VOID_RETURN", "TEST_BENCHMARK_REQUIRES_ZERO_PARAMETERS", "TEST_PROPHECY_REQUIRES_VOID_RETURN", "TEST_PROPHECY_REQUIRES_ZERO_PARAMETERS", "TEST_FORETOLD_REQUIRES_PROPHECY", "TEST_ARTIFACT_INVALID", "TEST_ARTIFACT_DUPLICATE", "TEST_ARTIFACT_MISSING", "TEST_ARTIFACT_OUTSIDE_ROOT", "TEST_ASSERT_UNKNOWN", "TEST_ASSERT_REASON_REQUIRED", "TEST_ASSERT_REASON_NONEMPTY", "TEST_ASSERT_BOOL_REQUIRED", "TEST_ASSERT_EQUALS_TYPE_MISMATCH", "TEST_ASSERT_EQUALS_UNSUPPORTED", "TEST_ASSERT_NEAR_NUMERIC_REQUIRED", "TEST_ASSERT_NEAR_NEGATIVE_TOLERANCE", "TEST_ASSERT_RESULT_REQUIRED", "TEST_FORETELL_UNKNOWN", "TEST_FORETELL_CHECKPOINT_INVALID":
		return d.Code
	case "PLAN_UNSUPPORTED_MIR", "PLAN_TARGET_UNSUPPORTED", "PLAN_REQUIRED_FACT_MISSING", "PLAN_REQUIRED_FACT_DISPROVEN", "PLAN_INVALID_STRATEGY", "PLAN_RUNTIME_GUARD_MISSING", "PLAN_CLEANUP_INCOMPLETE", "PLAN_WITNESS_INVALID", "PLAN_TENSOR_INVALID", "PLAN_ARTIFACT_INVALID", "PLAN_AUTOMATA_INVALID", "PLAN_CALLABLE_INVALID", "PLAN_CALLABLE_TYPE_INVALID":
		return "PLANNER_INVALID"
	case "CALLABLE_CAPTURE_REQUIRED", "CALLABLE_CAPTURE_DUPLICATE", "CALLABLE_CAPTURE_UNKNOWN", "CALLABLE_CAPTURE_COPY_NONCOPYABLE", "CALLABLE_CAPTURE_INVALID", "CALLABLE_CAPTURE_LIFETIME_INVALID", "CALLABLE_CONST_MUTATING_INVOKE", "CALLABLE_ARITY_MISMATCH", "CALLABLE_MIR_INVALID", "CALLABLE_TYPE_MIR_INVALID", "CALLABLE_TYPE_QUERY_INVALID", "CALLABLE_TYPE_ALIAS_DUPLICATE", "CALLABLE_RETURN_INFERENCE_INVALID", "CALLABLE_RETURN_TYPE_MISMATCH", "CALLABLE_AUTO_FIELD_EXISTENTIAL", "CALLABLE_FIELD_REF_ESCAPE", "CALLABLE_MACHINE_FIELD_SHORT_REF", "CALLBACK_SIGNATURE_INVALID", "CALLBACK_SIGNATURE_MISMATCH", "CALLBACK_OWNING_TEMPORARY_REJECTED", "CALLBACK_REQUIRES_EXPLICIT_BORROW", "CALLBACK_REQUIRES_CONCRETE_CALLABLE":
		return d.Code
	case "CV4024":
		return "UNKNOWN_NAME"
	case "CV4127":
...
func (d Diagnostic) Error() string {
	base := fmt.Sprintf("concept:%d:%d: %s: %s", d.Span.Line, d.Span.Column, d.Code, d.Message)
	if d.Proof != nil {
		return base + "\n\n" + RenderProofSummary(*d.Proof)
	}
	return base
}
