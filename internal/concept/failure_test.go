...
func TestFailureConformance(t *testing.T) {
	valid, invalid := 0, 0
	for _, tc := range failureCases {
		t.Run(tc.file, func(t *testing.T) {
			...
			path := filepath.Join("..", "..", "language", "evt1", "failure", dir, tc.file)
...
func TestFailureStaticAssertUsesComptimePath(t *testing.T) {
...
func TestFailureOwnedMatchRequiresMove(t *testing.T) {
...
func TestFailureOwnedPropagationRequiresMove(t *testing.T) {
...
func TestFailureResultVoidUsesCanonicalCarrier(t *testing.T) {
...
`)
	...
	runFoundationNativeHarness(t, outputs, "result_void_harness.c", "#include \"result_void.generated.h\"\nint main(void) { return concept_result_void_forward().tag == 0 ? 0 : 1; }\n")
}

func TestFailureTryBoundaryRunsLexicalCleanup(t *testing.T) {
	source := []byte(`profile Core;
...
`)
	...
	runFoundationNativeHarness(t, outputs, "try_cleanup_harness.c", "#include \"try_cleanup.generated.h\"\nint main(void) { return concept_try_cleanup_run(false) == 9 ? 0 : 1; }\n")
}
...
func TestFailureTerminalPanicPaths(t *testing.T) {
