...
var iterationInvalidFixtures = map[string]string{
...
func iterationFixture(t *testing.T, class, file string) Outputs {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1", "iteration", class, file)
...
func TestIterationYieldConformance(t *testing.T) {
	for _, file := range iterationValidFixtures {
		t.Run("valid/"+file, func(t *testing.T) { iterationFixture(t, "valid", file) })
	}
	for file, code := range iterationInvalidFixtures {
		t.Run("invalid/"+file, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1", "iteration", "invalid", file)
			source, err := os.ReadFile(path)
			if err != nil {
				t.Fatal(err)
			}
...
func TestIterationYieldNativeC11(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.file, func(t *testing.T) {
			outputs := iterationFixture(t, "valid", tc.file)
			...
			runFoundationNativeHarness(t, outputs, "iteration_harness.c", harness)
...
func TestIterationYieldMIRAndPlanner(t *testing.T) {
	yieldOutputs := iterationFixture(t, "valid", "yield_basic.concept")
	var yieldMIR MIR
	...
	foreachOutputs := iterationFixture(t, "valid", "foreach_array.concept")
	var eachMIR MIR
	...
	path := filepath.Join("..", "..", "language", "evt1", "iteration", "valid", "yield_basic.concept")
	...
	transientBody := string(iterationFixture(t, "valid", "yield_transient_owned_cleanup.concept")["yield_transient_owned_cleanup.generated.c"])
	...
	persistentBody := string(iterationFixture(t, "valid", "yield_persistent_owned_survives.concept")["yield_persistent_owned_survives.generated.c"])
