...
var evt1R4eConformanceCases = []ConformanceCase{
...
func TestEVT1R4eConformance(t *testing.T) {
	counts := map[ConformanceExpectation]int{}
	for _, tc := range evt1R4eConformanceCases {
		t.Run(tc.Name, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1-r4e", "core", filepath.FromSlash(tc.Source))
...
func TestEVT1R4eMIRAndDescriptorFacts(t *testing.T) {
...
func TestEVT1R4eNativeC11(t *testing.T) {
	cases := []struct {
		file, call string
		want       int
	}{
	...
	for _, tc := range cases {
		t.Run(tc.file, func(t *testing.T) {
			...
			runR1NativeHarness(t, outputs, "r4e_harness.c", harness)
		})
	}
}

func TestEVT1R4eRuntimeBindPanics(t *testing.T) {
	tests := []struct {
...
func TestEVT1R4eProvenanceSummaries(t *testing.T) {
...
func generateR4eFixture(t *testing.T, class, file string) Outputs {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1-r4e", "core", class, file)
	source, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
