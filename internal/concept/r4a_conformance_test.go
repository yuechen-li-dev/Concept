...
type r4aConformanceCase struct {
	name             string
	source           string
	accepted         bool
...
var evt1R4aConformanceCases = []r4aConformanceCase{
...
func TestEVT1R4aConformance(t *testing.T) {
	accepted, rejected := 0, 0
	for _, tc := range evt1R4aConformanceCases {
		t.Run(tc.name, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1-r4a", "core", filepath.FromSlash(tc.source))
...
func TestEVT1R4aMIRLifetimeAndProofFacts(t *testing.T) {
...
func TestEVT1R4aOwnedReplacementDropsOldFirst(t *testing.T) {
...
func TestEVT1R4aMIRCleanupValidationRejectsInconsistentObligations(t *testing.T) {
...
func TestEVT1R4aNativeC11(t *testing.T) {
	cases := []struct {
		fixture, call string
		want          int
	}{
...
func generateR4aFixture(t *testing.T, fixture string) Outputs {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1-r4a", "core", "valid", fixture)
	source, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
