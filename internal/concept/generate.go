...
func GenerateForTargetWithPolicy(module Module, source []byte, target TargetCapabilities, policy CompilationPolicy) (Outputs, error) {
	...
	if len(l.mir.Tests) > 0 {
		l.mapDoc["test_declarations"] = l.mir.Tests
	}
...
func buildMIR(module Module, env *semanticEnv) MIR {
	...
	for _, fn := range module.Functions {
		...
		entry := MIRTestDeclaration{Function: fn.Name, Kind: string(kind), SourceSpan: fn.Span, Artifacts: artifacts, Async: fn.Async, Foretold: foretold}
		for _, param := range fn.Params {
			entry.TheoryParameters = append(entry.TheoryParameters, TestParameter{Name: param.Name, Type: param.Type.String()})
		}
...
func collectExprMIROps(env *semanticEnv, expr Expr, fn *MIRFunction, templateInfo *evt1TemplateInfo) {
	...
	case *IntLiteral:
		fn.Operations = append(fn.Operations, MIROperation{ID: id, Kind: "literal", Detail: e.Source(), SourceSpan: e.Span})
	case *FloatLiteral:
...
func evt1MapFunctions(module Module, env *semanticEnv) []map[string]any {
	...
	for _, fn := range module.Functions {
		out = append(out, map[string]any{
			"name":   fn.Name,
			"span":   fn.Span,
			"symbol": evt1FunctionSymbolForDecl(evt1SemanticSymbolBase(module), env, fn),
		})
	}
	return out
}
...
func evt1SemanticSymbolBase(module Module) string {
	if module.Name == "" {
		return evt1OutputBase(module.Path)
	}
	// The complete declared module path, not the source location, owns the ABI
	// namespace. Dots are retained as explicit component boundaries.
	return evt1CName(strings.ReplaceAll(module.Name, ".", "__"))[len("concept_"):]
}
...
func evt1FunctionSymbolForDecl(base string, env *semanticEnv, fn FunctionDecl) string {
	...
	name := evt1FunctionSymbol(base, fn.Name)
	component := evt1CName(fn.Name)[len("concept_"):]
	for otherName := range env.functions {
		if otherName != fn.Name && evt1CName(otherName)[len("concept_"):] == component {
			name += "__n_" + fmt.Sprintf("%x", []byte(fn.Name))
			break
		}
	}
	...
	var parts []string
	for _, param := range fn.Params {
		parts = append(parts, evt1FunctionParamIdentity(evt1CanonicalType(env, param.Type)))
	}
	return name + "__" + strings.Join(parts, "_")
}

func evt1FunctionParamIdentity(t Type) string {
	// Qualifiers distinguish overloads, but must not alter structural type
	// identity used by runtime declaration dependency analysis.
	var qualifiers []string
	if t.Scoped {
		qualifiers = append(qualifiers, "scoped")
	}
	if t.Ownership != "" {
		qualifiers = append(qualifiers, t.Ownership)
	}
	if t.Const {
	...
	t.Const = false
	t.Scoped = false
	return strings.Join(append(qualifiers, evt1TypeIdentity(t)), "_")
}

func evt1ValidateGeneratedSymbolUniqueness(module Module, env *semanticEnv, base string) error {
	seen := map[string]FunctionDecl{}
	for _, fn := range module.Functions {
		symbol := evt1FunctionSymbolForDecl(base, env, fn)
		if prior, exists := seen[symbol]; exists {
			return evt1Diagnostic("CV4647", fmt.Sprintf("generated C symbol collision %s between %s and %s; declarations require distinct semantic identities", symbol, prior.Name, fn.Name), fn.Span)
		}
		seen[symbol] = fn
	}
	return nil
...
func (l *lowering) generateC() ([]byte, []byte, error) {
	if err := evt1ValidateGeneratedSymbolUniqueness(l.module, l.env, l.symbolBase); err != nil {
		return nil, nil, err
	}
	...
	if len(inferenceTypes) > 0 || evt1ModuleUsesTestNear(l.module) {
		body.WriteString("#include <math.h>\n")
	}
	if evt1ModuleUsesTestStringEquality(l.module) {
		body.WriteString("#include <string.h>\n")
	}
	body.WriteString("#include <stdio.h>\n#include <stdlib.h>\n\n")
	bodyPrefixLen := body.Len()
...
func (l *lowering) templateInstanceBody(instance *evt1TemplateInstance) string {
	if instance.Function.Async {
		...
		stepName := evt1AsyncStepName(l.symbolBase, fn.Name)
		initName := evt1AsyncInitName(l.symbolBase, fn.Name)
		var b strings.Builder
...
func (f *evt1FunctionLowerer) lowerExpr(expr Expr, indent int) (string, string, Type) {
	...
	case *IntLiteral:
		...
		return "", evt1RenderIntegerLiteral(e), t
	case *StringLiteral:
	...
	case *NameExpr:
		...
		if fns, ok := f.l.env.functions[e.Name]; ok && len(fns) == 1 {
			return "", evt1FunctionSymbolForDecl(f.l.symbolBase, f.l.env, fns[0]), fns[0].ReturnType
		}
		return "", e.Name, Type{}
	case *FieldExpr:
	...
	case *BinaryExpr:
		...
		if resultType.Name == "int" {
			switch e.Op {
			case "+":
				return leftPrelude + rightPrelude, fmt.Sprintf("%s(%s, %s, %d, %d)", evt1IntegerHelperName(f.l.symbolBase, "i32_add"), left, right, e.Span.Line, e.Span.Column), resultType
			case "-":
				return leftPrelude + rightPrelude, fmt.Sprintf("%s(%s, %s, %d, %d)", evt1IntegerHelperName(f.l.symbolBase, "i32_sub"), left, right, e.Span.Line, e.Span.Column), resultType
			case "*":
...
func evt1DropSymbol(l *lowering, dropFn FunctionDecl, t Type) string {
	...
	return evt1FunctionSymbolForDecl(l.symbolBase, l.env, dropFn)
}
