...
var evt1R3ConformanceCases = []r3ConformanceCase{
...
func TestEVT1R3DifferentialConformance(t *testing.T) {
	counts := map[ConformanceExpectation]int{}
	for _, tc := range evt1R3ConformanceCases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.reference == "" {
				t.Fatal("conformance provenance is required")
			}
			path := filepath.Join("..", "..", "language", "evt1-r3", "core", filepath.FromSlash(tc.source))
...
func TestEVT1R3ConformanceClassificationCounts(t *testing.T) {
	counts := map[ConformanceExpectation]int{}
	for _, tc := range evt1R3ConformanceCases {
		counts[tc.expected]++
	}
...
func TestEVT1R3MIRMoveReferenceAndCleanupFacts(t *testing.T) {
	path := filepath.Join("..", "..", "language", "evt1-r3", "core", "valid", "drop_moved_source.concept")
	source, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEVT1R3MoveAndReferenceSpans(t *testing.T) {
	movePath := filepath.Join("..", "..", "language", "evt1-r3", "core", "valid", "move_resource.concept")
	...
	refPath := filepath.Join("..", "..", "language", "evt1-r3", "core", "valid", "ref_mutable_parameter.concept")
...
func TestEVT1R3DropLoweringOrderAndTransfer(t *testing.T) {
...
func TestEVT1R3NativeC11(t *testing.T) {
	cases := []struct {
		fixture string
		call    string
		want    int
	...
	for _, tc := range cases {
		t.Run(tc.fixture, func(t *testing.T) {
			...
			runR1NativeHarness(t, outputs, base+"_harness.c", harness)
		})
	}
}
...
func generateR3Fixture(t *testing.T, fixture string) Outputs {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1-r3", "core", "valid", fixture)
	source, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
