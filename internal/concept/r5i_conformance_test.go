...
var evt1R5iInvalidCases = map[string]string{
...
func r5iFixture(t *testing.T, kind, name string) (Module, []byte) {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1-r5i", "core", kind, name)
...
func generateR5iFixture(t *testing.T, name string) (Module, Outputs) {
	t.Helper()
	module, source := r5iFixture(t, "valid", name)
...
func TestEVT1R5iCorpusConformance(t *testing.T) {
	valid, _ := filepath.Glob(filepath.Join("..", "..", "language", "evt1-r5i", "core", "valid", "*.concept"))
	invalid, _ := filepath.Glob(filepath.Join("..", "..", "language", "evt1-r5i", "core", "invalid", "*.concept"))
	...
	for _, path := range invalid {
		t.Run("invalid/"+filepath.Base(path), func(t *testing.T) {
			...
			if got, want := diagnosticCode(err), evt1R5iInvalidCases[filepath.Base(path)]; got != want {
...
func TestEVT1R5iMIRPlannerAndRepresentation(t *testing.T) {
...
func TestEVT1R5iExactEnvironmentGeometry(t *testing.T) {
...
func TestEVT1R5iMoveOnlyCallableTransferAndDropState(t *testing.T) {
	prefix := `profile Core;
...
func TestEVT1R5iLetVarCompatibilityAliases(t *testing.T) {
	source := `profile Core;
int Main() {
...
func TestEVT1R5iDeterminismAndForbiddenRuntime(t *testing.T) {
	module, source := r5iFixture(t, "valid", "callback_async_interface_composition.concept")
...
func TestEVT1R5iNativeC11(t *testing.T) {
	...
	for name, want := range cases {
		t.Run(name, func(t *testing.T) {
			...
			runR1NativeHarness(t, outputs, "r5i_"+name+"_harness.c", harness)
		})
	}
}
