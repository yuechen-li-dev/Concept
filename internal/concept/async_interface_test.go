...
var asyncInterfaceInvalidCases = map[string]string{
...
func asyncInterfaceFixture(t *testing.T, kind, name string) (Module, []byte) {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1", "async", "interface", kind, name)
...
func TestAsyncInterfaceCorpusConformance(t *testing.T) {
	valid, err := filepath.Glob(filepath.Join("..", "..", "language", "evt1", "async", "interface", "valid", "*.concept"))
	if err != nil {
		t.Fatal(err)
	}
	invalid, err := filepath.Glob(filepath.Join("..", "..", "language", "evt1", "async", "interface", "invalid", "*.concept"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, path := range invalid {
		t.Run(filepath.Base(path), func(t *testing.T) {
			...
			want := asyncInterfaceInvalidCases[filepath.Base(path)]
...
func TestAsyncInterfaceAliasesNormalizeIdentically(t *testing.T) {
	shortModule, _ := asyncInterfaceFixture(t, "valid", "dyn_async_method_await.concept")
	longModule, _ := asyncInterfaceFixture(t, "valid", "long_alias_async_interface.concept")
...
func TestAsyncInterfaceRejectsMalformedAsyncWitnessAndPlan(t *testing.T) {
