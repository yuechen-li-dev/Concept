...
var layoutStreamConformanceCases = []ConformanceCase{
...
func TestLayoutStreamConformance(t *testing.T) {
	counts := map[ConformanceExpectation]int{}
	for _, tc := range layoutStreamConformanceCases {
		t.Run(tc.Name, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1", "storage", "layout-stream", filepath.FromSlash(tc.Source))
