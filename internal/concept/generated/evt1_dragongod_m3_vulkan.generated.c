/* Generated by concept-evt1-stage0-go. DO NOT EDIT. Source: Examples/Concept-Vulkan/evt1_dragongod_m3_vulkan.concept */
...
enum {
  concept_buffer_lifecycle_state_main_ready = 0,
  concept_buffer_lifecycle_state_main_deferred = 1,
  concept_buffer_lifecycle_state_main_submitting = 2,
  concept_buffer_lifecycle_state_main_active = 3,
  concept_buffer_lifecycle_state_main_failed = 4,
  concept_buffer_lifecycle_state_main_finished = 5,
};
...
typedef enum concept_buffer_lifecycle_effect_tag {
  CONCEPT_BUFFER_LIFECYCLE_EFFECT_RECORD_BUFFER_SUBMISSION = 0,
  CONCEPT_BUFFER_LIFECYCLE_EFFECT_BEGIN_BUFFER_SUBMISSION = 1,
  CONCEPT_BUFFER_LIFECYCLE_EFFECT_MARK_BUFFER_FAILURE = 2,
} concept_buffer_lifecycle_effect_tag;

typedef enum concept_buffer_lifecycle_effect_batch_state {
  CONCEPT_BUFFER_LIFECYCLE_EFFECT_BATCH_VACANT = 0,
  CONCEPT_BUFFER_LIFECYCLE_EFFECT_BATCH_PENDING = 1,
  CONCEPT_BUFFER_LIFECYCLE_EFFECT_BATCH_COMPLETED = 2,
  CONCEPT_BUFFER_LIFECYCLE_EFFECT_BATCH_FAILED = 3,
} concept_buffer_lifecycle_effect_batch_state;

typedef struct concept_buffer_lifecycle_effect_entry {
...
typedef struct concept_buffer_lifecycle_effects {
  concept_buffer_lifecycle_effect_batch_state state;
  uint8_t count;
  uint8_t cursor;
  uint8_t failed_index;
  concept_buffer_lifecycle_effect_entry entries[2];
} concept_buffer_lifecycle_effects;

static void concept_buffer_lifecycle_effects_discard(concept_buffer_lifecycle_effects* batch) {
  batch->state = CONCEPT_BUFFER_LIFECYCLE_EFFECT_BATCH_VACANT;
  batch->count = 0;
  batch->cursor = 0;
  batch->failed_index = 0;
}

static concept_automata_dispatch_outcome concept_buffer_lifecycle_dispatch(concept_buffer_lifecycle_instance* instance, concept_resource_signal signal, concept_buffer_lifecycle_effects* batch) {
  if (instance->finished) {
    concept_buffer_lifecycle_effects_discard(batch);
    return concept_automata_dispatch_outcome_make_already_finished();
  }
  if (batch->state == CONCEPT_BUFFER_LIFECYCLE_EFFECT_BATCH_PENDING || batch->state == CONCEPT_BUFFER_LIFECYCLE_EFFECT_BATCH_FAILED) {
    return concept_automata_dispatch_outcome_make_effect_batch_occupied();
  }
  concept_buffer_lifecycle_effects staged_batch = {0};
  staged_batch.state = CONCEPT_BUFFER_LIFECYCLE_EFFECT_BATCH_PENDING;
  uint8_t staged_count = 0;
  concept_buffer_lifecycle_instance staged_instance = *instance;
  concept_buffer_lifecycle_instance* staged = &staged_instance;
  switch (staged->current_machine) {
    case concept_buffer_lifecycle_machine_main:
      switch (staged->current_state) {
        case concept_buffer_lifecycle_state_main_ready:
          switch (signal.tag) {
            case CONCEPT_RESOURCE_SIGNAL_SUBMIT:
              ...
              if (concept_evt1_dragongod_m3_vulkan_fail_requested(staged->context)) {
                eligible_count = (uint8_t)(eligible_count + 1);
              ...
              switch (selected_candidate) {
                case 1:
                  ...
                  staged_batch.entries[staged_count].tag = CONCEPT_BUFFER_LIFECYCLE_EFFECT_BEGIN_BUFFER_SUBMISSION;
                  staged_count = (uint8_t)(staged_count + 1);
                  staged->current_state = concept_buffer_lifecycle_state_main_submitting;
                    break;
                  }
                case 2:
                  {
                ...
                case 3:
                  ...
                  staged_batch.entries[staged_count].tag = CONCEPT_BUFFER_LIFECYCLE_EFFECT_MARK_BUFFER_FAILURE;
                  staged_count = (uint8_t)(staged_count + 1);
                  staged->current_state = concept_buffer_lifecycle_state_main_deferred;
                    break;
                  }
                default:
                  batch->count = 0;
        ...
        case concept_buffer_lifecycle_state_main_finished:
          concept_abort_invalid_automata_state("BufferLifecycle", staged->current_machine, staged->current_state);
          batch->count = 0;
          return concept_automata_dispatch_outcome_make_already_finished();
        default:
          concept_abort_invalid_automata_state("BufferLifecycle", staged->current_machine, staged->current_state);
...
int concept_evt1_dragongod_m3_vulkan_vulkan_effectful_submit_code(concept_buffer_context context, VkBuffer buffer) {
  ...
  concept_automata_dispatch_outcome cv_arg_02 = concept_buffer_lifecycle_dispatch(&lifecycle, cv_signal_01, &emitted);
...
int concept_evt1_dragongod_m3_vulkan_vulkan_zero_emit_transition_code(concept_buffer_context context, VkCommandPool pool) {
  ...
  concept_buffer_lifecycle_effects emitted = {0};
  concept_resource_signal cv_signal_01 = concept_resource_signal_make_submit();
  (void)concept_buffer_lifecycle_dispatch(&lifecycle, cv_signal_01, &emitted);
  concept_resource_signal cv_signal_02 = concept_resource_signal_make_submitted();
  concept_automata_dispatch_outcome cv_arg_03 = concept_buffer_lifecycle_dispatch(&lifecycle, cv_signal_02, &emitted);
  return concept_evt1_dragongod_m3_vulkan_outcome_code(cv_arg_03);
}
