...
var evt1R5bInvalid = map[string]string{
...
func r5bFixture(t *testing.T, class, file string) Outputs {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1-r5b", "core", class, file)
...
func TestEVT1R5bConformance(t *testing.T) {
	for _, file := range evt1R5bValid {
		t.Run("valid/"+file, func(t *testing.T) { r5bFixture(t, "valid", file) })
	}
	for file, code := range evt1R5bInvalid {
		t.Run("invalid/"+file, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1-r5b", "core", "invalid", file)
			source, err := os.ReadFile(path)
			if err != nil {
				t.Fatal(err)
			}
...
func TestEVT1R5bNativeC11(t *testing.T) {
	cases := []struct {
		file string
		want int
	}{
	...
	for _, tc := range cases {
		t.Run(tc.file, func(t *testing.T) {
			outputs := r5bFixture(t, "valid", tc.file)
			...
			runR1NativeHarness(t, outputs, "r5b_harness.c", harness)
...
func TestEVT1R5bMIRAndPlanner(t *testing.T) {
	outputs := r5bFixture(t, "valid", "transition_evaluation_counts.concept")
	var mir MIR
	...
	path := filepath.Join("..", "..", "language", "evt1-r5b", "core", "valid", "transition_evaluation_counts.concept")
...
func TestEVT1R5bEvaluationAndCleanupShape(t *testing.T) {
	evidence := string(r5bFixture(t, "valid", "transition_evaluation_counts.concept")["transition_evaluation_counts.generated.c"])
	...
	cleanup := string(r5bFixture(t, "valid", "transition_match_updates_state.concept")["transition_match_updates_state.generated.c"])
	...
	decideCleanup := string(r5bFixture(t, "valid", "transition_decide_int_scores.concept")["transition_decide_int_scores.generated.c"])
...
func TestEVT1R5bTerminalPanicPaths(t *testing.T) {
	cases := []struct {
	...
	for _, tc := range cases {
		t.Run(tc.file, func(t *testing.T) {
			...
			outputs := r5bFixture(t, "invalid", tc.file)
			base := strings.TrimSuffix(tc.file, ".concept")
			dir := t.TempDir()
