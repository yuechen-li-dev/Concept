...
var storageArrayConformanceCases = []ConformanceCase{
...
func TestStorageArraysConformance(t *testing.T) {
	counts := map[ConformanceExpectation]int{}
	for _, tc := range storageArrayConformanceCases {
		t.Run(tc.Name, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1", "storage", "arrays", filepath.FromSlash(tc.Source))
...
func TestStorageArraysRuntimeBoundsPanic(t *testing.T) {
