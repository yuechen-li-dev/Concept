...
var foundationConformanceCases = []ConformanceCase{
...
func TestFoundationDifferentialConformance(t *testing.T) {
	counts := map[ConformanceExpectation]int{}
	for _, tc := range foundationConformanceCases {
		t.Run(tc.Name, func(t *testing.T) {
			if tc.Area == "" || tc.Reference == "" || tc.MatrixStatus == "" || tc.Notes == "" {
				t.Fatal("conformance metadata is incomplete")
			}
			path := filepath.Join("..", "..", "language", "evt1", "foundation", filepath.FromSlash(tc.Source))
...
func TestFoundationConformanceClassificationCounts(t *testing.T) {
	counts := map[ConformanceExpectation]int{}
	for _, tc := range foundationConformanceCases {
		counts[tc.Expected]++
	}
...
func TestFoundationCoreStructsNativeC11(t *testing.T) {
	path := filepath.Join("..", "..", "language", "evt1", "foundation", "valid", "structs.concept")
	source, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
