...
type QualifiedEnumMember struct {
...
const (
	TransitionGoto TransitionKind = "goto"
	TransitionPush TransitionKind = "push"
)
...
type TransitionDecl struct {
	Signal       QualifiedEnumMember `json:"signal"`
	Guard        Expr                `json:"guard,omitempty"`
	Otherwise    bool                `json:"otherwise,omitempty"`
	Emits        []EmitStmt          `json:"emits,omitempty"`
	Kind         TransitionKind      `json:"kind"`
	TargetState  StateRef            `json:"target_state,omitempty"`
	PushMachine  string              `json:"push_machine,omitempty"`
	Continuation StateRef            `json:"continuation_state,omitempty"`
	Span         Span                `json:"span"`
}

type CompletionDecl struct {
...
type MIRTransition struct {
	...
	Emits             []MIREmit `json:"emits,omitempty"`
	SourceSpan        Span      `json:"source_span"`
}
...
func evt1ValidateAutomataPushCycles(info *evt1AutomataInfo) error {
	...
	for _, machine := range info.Decl.Machines {
		for _, state := range machine.States {
			for _, handler := range state.Handlers {
				if handler.Kind != TransitionPush {
					continue
				}
...
func evt1LongestPushDepth(info *evt1AutomataInfo, machineIndex map[string]MachineDecl, memo map[string]int) (int, error) {
	...
	visit = func(name string) (int, error) {
		...
		for _, state := range machine.States {
			...
			for _, handler := range state.Handlers {
				if handler.Kind != TransitionPush || !info.MachineReachable[handler.PushMachine] {
					continue
				}
...
func evt1LookupState(machine MachineDecl, name string) StateDecl {
	for _, state := range machine.States {
		if state.Name == name {
			return state
		}
...
func evt1AutomataEffectIdentity(env *semanticEnv, info *evt1AutomataInfo) string {
	var b strings.Builder
	b.WriteString(info.Decl.Name)
...
func evt1AutomataEffectSet(env *semanticEnv, info *evt1AutomataInfo) []string {
...
func evt1ValidateAutomataEmits(env *semanticEnv, info *evt1AutomataInfo, handler TransitionDecl) error {
...
func evt1ValidateEffectPayloadExpr(env *semanticEnv, scope *evt1Scope, expr Expr) error {
	if _, err := validateExpr(env, scope, expr, nil, false); err != nil {
		return err
	}
	switch e := expr.(type) {
	case *IntLiteral, *BoolLiteral, *NameExpr:
		return nil
	case *ParenExpr:
...
func evt1ValidateAutomataCandidateGroups(env *semanticEnv, info *evt1AutomataInfo, machine MachineDecl, state StateDecl) error {
