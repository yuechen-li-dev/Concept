...
func TestDragonGodM1GenerationIncludesRuntimeDispatch(t *testing.T) {
	...
	for _, needle := range []string{
		...
		"concept_resource_lifecycle_dispatch(",
		"concept_resource_lifecycle_normalize(",
...
func TestDragonGodM3GenerationIncludesEffectBatchStaging(t *testing.T) {
	...
	for _, needle := range []string{
		...
		"*batch = staged_batch;",
		"batch->count = 0;",
		"CONCEPT_RESOURCE_LIFECYCLE_EFFECT_RECORD_SUBMISSION",
		"CONCEPT_RESOURCE_LIFECYCLE_EFFECT_BEGIN_SUBMISSION",
	} {
		if !strings.Contains(body, needle) {
			t.Fatalf("generated C missing %q\n%s", needle, body)
		}
...
func TestDragonGodM4SpecimensParseAndGenerate(t *testing.T) {
	for _, fixture := range []string{
		...
		t.Run(fixture, func(t *testing.T) {
			...
			if _, err := Generate(module, []byte(src)); err != nil {
				t.Fatal(err)
			}
		})
	}
...
int main(void) {
  concept_buffer_context unique = {true, false};
  concept_buffer_context fallback = {false, false};
...
int main(void) {
  ...
  concept_lifecycle_context fallback = {false, false, 12, concept_queue_class_make_compute(), 99};
  concept_lifecycle_context ambiguous = {true, true, 8, concept_queue_class_make_graphics(), 55};
  concept_resource_lifecycle_instance lifecycle;
  concept_resource_lifecycle_instance fallbackLifecycle;
  concept_resource_lifecycle_instance ambiguousLifecycle;
  concept_resource_lifecycle_effects emitted = {0};
  concept_resource_lifecycle_effects fallbackBatch = {0};
  concept_resource_lifecycle_effects ambiguousBatch = {0};
  concept_automata_dispatch_outcome a;
  concept_automata_dispatch_outcome b;
  concept_automata_dispatch_outcome c;
  concept_automata_dispatch_outcome d;
  concept_automata_dispatch_outcome e;
  concept_automata_dispatch_outcome f;

  concept_resource_lifecycle_init(&lifecycle, &unique);
  a = concept_resource_lifecycle_dispatch(&lifecycle, concept_lifecycle_signal_make_submit(), &emitted);
  if (a.tag != CONCEPT_AUTOMATA_DISPATCH_OUTCOME_TRANSITIONED) return 1;
  if (emitted.count != 2) return 2;
  if (emitted.entries[0].tag != CONCEPT_RESOURCE_LIFECYCLE_EFFECT_RECORD_SUBMISSION) return 3;
  if (emitted.entries[0].payload.record_submission.submission != 41) return 4;
  if (emitted.entries[1].tag != CONCEPT_RESOURCE_LIFECYCLE_EFFECT_BEGIN_SUBMISSION) return 5;
  if (emitted.entries[1].payload.begin_submission.submission != 41) return 6;
  if (emitted.entries[1].payload.begin_submission.queue.tag != CONCEPT_QUEUE_CLASS_GRAPHICS) return 7;

  b = concept_resource_lifecycle_dispatch(&lifecycle, concept_lifecycle_signal_make_submitted(), &emitted);
  if (b.tag != CONCEPT_AUTOMATA_DISPATCH_OUTCOME_TRANSITIONED) return 8;
  if (emitted.count != 0) return 9;

  concept_resource_lifecycle_init(&fallbackLifecycle, &fallback);
  c = concept_resource_lifecycle_dispatch(&fallbackLifecycle, concept_lifecycle_signal_make_submit(), &fallbackBatch);
  if (c.tag != CONCEPT_AUTOMATA_DISPATCH_OUTCOME_TRANSITIONED) return 10;
  if (fallbackBatch.count != 1) return 11;
  if (fallbackBatch.entries[0].tag != CONCEPT_RESOURCE_LIFECYCLE_EFFECT_FINALIZE_TICKET) return 12;
  if (fallbackBatch.entries[0].payload.finalize_ticket.ticket != 99) return 13;

  concept_resource_lifecycle_init(&ambiguousLifecycle, &ambiguous);
  d = concept_resource_lifecycle_dispatch(&ambiguousLifecycle, concept_lifecycle_signal_make_submit(), &ambiguousBatch);
  if (d.tag != CONCEPT_AUTOMATA_DISPATCH_OUTCOME_AMBIGUOUS) return 14;
  if (ambiguousBatch.count != 0) return 15;

  e = concept_resource_lifecycle_dispatch(&ambiguousLifecycle, concept_lifecycle_signal_make_finish_now(), &ambiguousBatch);
  if (e.tag != CONCEPT_AUTOMATA_DISPATCH_OUTCOME_FINISHED) return 16;
  if (ambiguousBatch.count != 1) return 17;
  if (ambiguousBatch.entries[0].payload.finalize_ticket.ticket != 55) return 18;

  f = concept_resource_lifecycle_dispatch(&ambiguousLifecycle, concept_lifecycle_signal_make_submit(), &ambiguousBatch);
...
int main(void) {
  concept_buffer_context unique = {true, false, 21, concept_queue_class_make_graphics(), 5};
  concept_buffer_context fallback = {false, false, 22, concept_queue_class_make_transfer(), 8};
  concept_buffer_context ambiguous = {true, true, 23, concept_queue_class_make_graphics(), 13};
  concept_buffer_lifecycle_instance lifecycle;
  concept_buffer_lifecycle_instance fallbackLifecycle;
  concept_buffer_lifecycle_instance ambiguousLifecycle;
  concept_buffer_lifecycle_effects emitted = {0};
  concept_buffer_lifecycle_effects fallbackBatch = {0};
  concept_buffer_lifecycle_effects ambiguousBatch = {0};
  concept_automata_dispatch_outcome a;
  concept_automata_dispatch_outcome b;
  concept_automata_dispatch_outcome c;
  VkBuffer buffer = (VkBuffer)(uintptr_t)0x10u;
  ...
  concept_buffer_lifecycle_init(&lifecycle, &unique);
  a = concept_buffer_lifecycle_dispatch(&lifecycle, concept_resource_signal_make_submit(), &emitted);
  if (a.tag != CONCEPT_AUTOMATA_DISPATCH_OUTCOME_TRANSITIONED) return 1;
  if (emitted.count != 2) return 2;
  if (emitted.entries[0].payload.record_buffer_submission.bufferId != 21) return 3;
  if (emitted.entries[1].payload.begin_buffer_submission.queue.tag != CONCEPT_QUEUE_CLASS_GRAPHICS) return 4;

  concept_buffer_lifecycle_init(&fallbackLifecycle, &fallback);
  b = concept_buffer_lifecycle_dispatch(&fallbackLifecycle, concept_resource_signal_make_submit(), &fallbackBatch);
  if (b.tag != CONCEPT_AUTOMATA_DISPATCH_OUTCOME_TRANSITIONED) return 5;
  if (fallbackBatch.count != 1) return 6;
  if (fallbackBatch.entries[0].payload.mark_buffer_failure.failureCode != 8) return 7;

  concept_buffer_lifecycle_init(&ambiguousLifecycle, &ambiguous);
  c = concept_buffer_lifecycle_dispatch(&ambiguousLifecycle, concept_resource_signal_make_submit(), &ambiguousBatch);
  if (c.tag != CONCEPT_AUTOMATA_DISPATCH_OUTCOME_AMBIGUOUS) return 8;
...
static concept_result_void_resource_actuation_error err_result(concept_resource_actuation_error error) {
  concept_result_void_resource_actuation_error out = {0};
  out.is_error = true;
  out.error = error;
  return out;
...
int main(void) {
  concept_lifecycle_context ready = {true, false, 41, concept_queue_class_make_graphics(), 7};
  concept_lifecycle_context fallback = {false, false, 12, concept_queue_class_make_compute(), 99};
  concept_resource_mechanism mechanism = {-1, 0, 0, 0, 0};
  concept_resource_mechanism failureMechanism = {1, 0, 0, 0, 0};
  concept_resource_lifecycle_instance lifecycle;
  concept_resource_lifecycle_effects emitted = {0};
  concept_resource_lifecycle_effects failedBatch = {0};
  concept_resource_lifecycle_executor executor;
  concept_resource_lifecycle_executor failedExecutor;
  concept_automata_dispatch_outcome dispatchOutcome;
  concept_actuation_result__resource_lifecycle_actuator successResult;
  concept_actuation_result__resource_lifecycle_actuator repeatResult;
  concept_actuation_result__resource_lifecycle_actuator noBatchResult;
  concept_actuation_result__resource_lifecycle_actuator failedResult;

  concept_resource_lifecycle_init(&lifecycle, &ready);
  dispatchOutcome = concept_resource_lifecycle_dispatch(&lifecycle, concept_lifecycle_signal_make_submit(), &emitted);
  if (dispatchOutcome.tag != CONCEPT_AUTOMATA_DISPATCH_OUTCOME_TRANSITIONED) return 1;
  if (emitted.state != CONCEPT_RESOURCE_LIFECYCLE_EFFECT_BATCH_PENDING) return 2;
  if (concept_resource_lifecycle_dispatch(&lifecycle, concept_lifecycle_signal_make_submitted(), &emitted).tag != CONCEPT_AUTOMATA_DISPATCH_OUTCOME_EFFECT_BATCH_OCCUPIED) return 3;

  concept_resource_lifecycle_executor_init(&executor, &mechanism);
  successResult = concept_resource_lifecycle_actuator_actuate(&emitted, &executor);
  if (successResult.outcome.tag != CONCEPT_ACTUATION_OUTCOME_COMPLETED) return 4;
  if (successResult.completedCount != 2) return 5;
  if (emitted.state != CONCEPT_RESOURCE_LIFECYCLE_EFFECT_BATCH_COMPLETED) return 6;
  if (mechanism.count != 2 || mechanism.observed0 != 41 || mechanism.observed1 != 41) return 7;

  repeatResult = concept_resource_lifecycle_actuator_actuate(&emitted, &executor);
  if (repeatResult.outcome.tag != CONCEPT_ACTUATION_OUTCOME_ALREADY_CONSUMED) return 8;
  ...
  concept_resource_lifecycle_executor_init(&failedExecutor, &failureMechanism);
  failedResult = concept_resource_lifecycle_actuator_actuate(&failedBatch, &failedExecutor);
  if (failedResult.outcome.tag != CONCEPT_ACTUATION_OUTCOME_FAILED) return 12;
  if (failedResult.failedIndex != 1) return 13;
  if (failedBatch.state != CONCEPT_RESOURCE_LIFECYCLE_EFFECT_BATCH_FAILED) return 14;
