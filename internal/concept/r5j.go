...
// evt1PrepareExactCallableTypes is the bounded R5j type-surface prepass. It
...
func evt1PrepareExactCallableTypes(env *semanticEnv, module *Module, typeNames map[string]Span) error {
	...
	for i := range module.TypeAliases {
		...
		if queryType.Kind != TypeCallable {
			return evt1Diagnostic("CALLABLE_TYPE_QUERY_INVALID", fmt.Sprintf("R5j exact type aliases require a concrete callable expression, got %s", queryType.String()), decl.Span)
		}
