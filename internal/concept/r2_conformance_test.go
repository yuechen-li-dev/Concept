...
var evt1R2ConformanceCases = []r2ConformanceCase{
...
func TestEVT1R2DifferentialConformance(t *testing.T) {
	counts := map[ConformanceExpectation]int{}
	for _, tc := range evt1R2ConformanceCases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.reference == "" {
				t.Fatal("conformance provenance is required")
			}
			path := filepath.Join("..", "..", "language", "evt1-r2", "core", filepath.FromSlash(tc.source))
...
func TestEVT1R2ConformanceClassificationCounts(t *testing.T) {
	counts := map[ConformanceExpectation]int{}
	for _, tc := range evt1R2ConformanceCases {
		counts[tc.expected]++
	}
...
func TestEVT1R2RecordMIRAndLowering(t *testing.T) {
	path := filepath.Join("..", "..", "language", "evt1-r2", "core", "valid", "record_with_update.concept")
	source, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEVT1R2SyntaxSpans(t *testing.T) {
	path := filepath.Join("..", "..", "language", "evt1-r2", "core", "valid", "record_with_update.concept")
	source, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
...
func TestEVT1R2WithBaseIsLoweredOnce(t *testing.T) {
...
func TestEVT1R2NativeC11(t *testing.T) {
	cases := []struct {
		fixture string
		call    string
		want    int
	...
	for _, tc := range cases {
		t.Run(tc.fixture, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1-r2", "core", "valid", tc.fixture)
			source, err := os.ReadFile(path)
			if err != nil {
				t.Fatal(err)
			}
