...
var evt1R5eInvalid = map[string]string{
...
func r5eFixture(t *testing.T, class, file string) Outputs {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1-r5e", "core", class, file)
...
func TestEVT1R5eConformance(t *testing.T) {
	for _, file := range evt1R5eValid {
		t.Run("valid/"+file, func(t *testing.T) { r5eFixture(t, "valid", file) })
	}
	for file, code := range evt1R5eInvalid {
		t.Run("invalid/"+file, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1-r5e", "core", "invalid", file)
			source, err := os.ReadFile(path)
			if err != nil {
				t.Fatal(err)
			}
	...
	r5eFixture(t, "invalid", "machine_stack_overflow.concept")
	r5eFixture(t, "invalid", "machine_result_before_completion.concept")
}

func TestEVT1R5eNativeC11(t *testing.T) {
	cases := []struct {
		file, call string
		want       int
	}{
	...
	for _, tc := range cases {
		t.Run(tc.file, func(t *testing.T) {
			outputs := r5eFixture(t, "valid", tc.file)
			...
			runR1NativeHarness(t, outputs, "r5e_harness.c", harness)
...
func TestEVT1R5eMIRAndPlanner(t *testing.T) {
	outputs := r5eFixture(t, "valid", "machine_child_success.concept")
	var mir MIR
	...
	path := filepath.Join("..", "..", "language", "evt1-r5e", "core", "valid", "machine_child_success.concept")
...
func TestEVT1R5eOwnedOutcomeTransferShape(t *testing.T) {
	neutral := string(r5eFixture(t, "valid", "machine_owned_child_cleanup.concept")["machine_owned_child_cleanup.generated.c"])
	...
	body := string(r5eFixture(t, "valid", "machine_owned_completion_transfer.concept")["machine_owned_completion_transfer.generated.c"])
