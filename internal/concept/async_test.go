...
func asyncFixture(t *testing.T, kind, name string) (Module, []byte) {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1", "async", "basic", kind, name)
...
func TestAsyncRuntimeStackOverflow(t *testing.T) {
	module, source := asyncFixture(t, "invalid", "async_stack_overflow.concept")
...
func TestAsyncRuntimeResultBeforeCompletion(t *testing.T) {
	module, source := asyncFixture(t, "invalid", "async_result_before_completion.concept")
	outputs, err := Generate(module, source)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAsyncStaticDiagnostics(t *testing.T) {
	...
	for file, code := range cases {
		path := filepath.Join("..", "..", "language", "evt1", "async", "basic", "invalid", file)
		source, err := os.ReadFile(path)
		if err != nil {
			t.Fatal(err)
		}
...
func TestAsyncBasicMIRPlanAndC(t *testing.T) {
	module, source := asyncFixture(t, "valid", "async_basic.concept")
	outputs, err := Generate(module, source)
	if err != nil {
		t.Fatal(err)
	}
	...
	runFoundationNativeHarness(t, outputs, "async_harness.c", harness)
}
...
func TestAsyncAliasesAndNestedNativeC11(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		runFoundationNativeHarness(t, tc.outputs, tc.name+"_harness.c", harness)
	}
	nested, nestedSource := asyncFixture(t, "valid", "async_nested_child.concept")
	nestedOutputs, err := Generate(nested, nestedSource)
	if err != nil {
		t.Fatal(err)
	}
	runFoundationNativeHarness(t, nestedOutputs, "nested_harness.c", "#include \"async_nested_child.generated.h\"\nint main(void) { return concept_async_nested_child_main(); }\n")
	for _, name := range []string{"async_named_operation", "async_result_question"} {
		module, source := asyncFixture(t, "valid", name+".concept")
		outputs, err := Generate(module, source)
		if err != nil {
			t.Fatal(err)
		}
...
func TestAsyncCorpusLivenessCleanupAndMutationGuards(t *testing.T) {
	valid, err := filepath.Glob(filepath.Join("..", "..", "language", "evt1", "async", "basic", "valid", "*.concept"))
	if err != nil {
		t.Fatal(err)
	}
	invalid, err := filepath.Glob(filepath.Join("..", "..", "language", "evt1", "async", "basic", "invalid", "*.concept"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for file, function := range map[string]string{
		...
		module, _ := asyncFixture(t, "valid", file)
	...
	ownedModule, ownedSource := asyncFixture(t, "valid", "async_owned_local_across_await.concept")
