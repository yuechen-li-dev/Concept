...
var evt1R5cInvalid = map[string]string{
...
func r5cFixture(t *testing.T, class, file string) Outputs {
	t.Helper()
	path := filepath.Join("..", "..", "language", "evt1-r5c", "core", class, file)
...
func TestEVT1R5cConformance(t *testing.T) {
	for _, file := range evt1R5cValid {
		t.Run("valid/"+file, func(t *testing.T) { r5cFixture(t, "valid", file) })
	}
	for file, code := range evt1R5cInvalid {
		t.Run("invalid/"+file, func(t *testing.T) {
			path := filepath.Join("..", "..", "language", "evt1-r5c", "core", "invalid", file)
			source, err := os.ReadFile(path)
			if err != nil {
				t.Fatal(err)
			}
...
func TestEVT1R5cNativeC11(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.file, func(t *testing.T) {
			outputs := r5cFixture(t, "valid", tc.file)
			...
			runR1NativeHarness(t, outputs, "r5c_harness.c", harness)
...
func TestEVT1R5cMIRPlannerAndLowering(t *testing.T) {
	outputs := r5cFixture(t, "valid", "infer_basic.concept")
	var mir MIR
	...
	path := filepath.Join("..", "..", "language", "evt1-r5c", "core", "valid", "infer_basic.concept")
	...
	transition := r5cFixture(t, "valid", "transition_infer_hardmax.concept")
	...
	cleanupBody := string(r5cFixture(t, "valid", "transition_infer_cleanup.concept")["transition_infer_cleanup.generated.c"])
...
func TestEVT1R5cTerminalPanicPaths(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.file, func(t *testing.T) {
			...
			outputs := r5cFixture(t, "invalid", tc.file)
			base := strings.TrimSuffix(tc.file, ".concept")
			dir := t.TempDir()
...
func TestEVT1R5cSoftmaxReference(t *testing.T) {
