...
| `yield` statement in machines | 🔶 | Phase 19 M3 reserves `yield`, validates bare `yield;` only in machine state bodies, lowers it in generated machine `Step` code to immediate `return;`, preserves state, leaves the machine incomplete, avoids result writes, panic, allocation, scheduler/async hooks, and pins twenty-one validation/runtime/backend fixtures; continuation yield and lifted locals remain deferred |
...
Stage 1 gaps include Phase 19 diagnostics/examples/closeout for machine `yield;`, visibility and package/filesystem
...
### Phase 19 M4-M6 yield interaction fixture sweep

P19-M4, P19-M5, and P19-M6 were completed together as a fixture sweep, raising the Phase 19 set to twenty-one fixtures under `language/phase19-yielding-machines/`: ten parse/check validation fixtures, nine runtime fixtures, and two backend C fixtures. Runtime coverage proves that `yield;` leaves `Complete(machine)` false, preserves `State(machine)`, re-enters the same state body from the beginning on the next `Step`, works as a terminal inside an `if` branch, keeps `Result(machine)` after yield on the existing shared `cpt_panic` result-before-completion path with exit code 101, allows nested child machines to advance across parent yields, leaves `State(child)` readable after parent yield, supports repeated external polling, and composes with later non-yielding `transition match` and `transition decide` paths. Backend coverage pins direct `return;` emission and absence of yield-only state assignment, completion writes, result writes, panic calls, allocation, scheduler/async/event/mailbox/blackboard markers, and DragonGod hooks. No new semantics were added; diagnostics/runtime hardening, examples, and closeout remain.
