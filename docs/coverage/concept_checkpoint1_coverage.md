...
## Phase 15 M2 vs PoC3 Constitution

**Generated:** June 2026  
**Compiler:** Stage 0 (Zig, self-hosted Concept frontend, C backend via MIR)  
**Phases closed:** 1 through 14
**Current phase:** Phase 15 M2 extern C HIR declarations and ABI type validation
...
| `extern "C"` declarations | 🔶 | Phase 15 M2 parses block-form declarations, lowers valid extern C functions to HIR with ABI/linkage metadata and C symbol names, validates the strict v0 ABI type subset, makes declarations visible to ordinary call resolution, and rejects duplicate extern C symbols. MIR/backend extern call/prototype/link behavior remains deferred |
...
- `extern "C"` MIR/backend call and prototype implementation after the Phase 15 M2 HIR/ABI validation milestone
...
| §36 C interop | 🔶 C backend done; Phase 15 M0 design added for `extern "C"`, `export "C"`, and `repr(C)`; P15-M2 parses and preserves `extern "C"` blocks, lowers valid extern functions into HIR with C ABI metadata, validates ABI types, and rejects duplicate C symbols while MIR/backend extern call/prototype behavior remains deferred |
...
through Phase 15 M2. Stage 1 is substantially implemented. Phase 14
...
validation while MIR/backend extern call and prototype emission remain
deferred. The critical path to full Stage 1 completion now includes C ABI implementation,
