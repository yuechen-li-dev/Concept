...
### Phase 19 M3 yield runtime/backend coverage

P19-M3 raises the Phase 19 set to sixteen fixtures under `language/phase19-yielding-machines/`: the ten P19-M2 parse/check validation fixtures plus five runtime fixtures and one backend C fixture. Runtime coverage proves that `yield;` leaves `Complete(machine)` false, preserves `State(machine)`, re-enters the same state body from the beginning on the next `Step`, works as a terminal inside an `if` branch, and keeps `Result(machine)` after yield on the existing shared `cpt_panic` result-before-completion path with exit code 101. Backend coverage pins direct `return;` emission and the absence of yield-only state assignment, completion writes, result writes, panic calls, allocation, scheduler/async/event/mailbox/blackboard markers, and DragonGod hooks. Continuation yield, lifted locals, sub-state program counters, `yield return`, yield values, `suspend`, `resume`, scheduler integration, and generator/async protocols remain out of scope.
