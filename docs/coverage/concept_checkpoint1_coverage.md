...
| `yield` statement in machines | 🔶 | Phase 19 M2 reserves `yield`, parses bare `yield;` in machine state bodies, accepts nested-block and `if`-branch state yields, rejects ordinary-function `yield;` with `CON0300`, rejects expression/value/`yield return` forms with `CON0301`/`CON0302`/`CON0303`, and pins ten parse/check fixtures; backend/runtime behavior is deferred to P19-M3 |
...
### Phase 19 M2 yield validation coverage

P19-M2 raises the Phase 19 set to ten fixtures under `language/phase19-yielding-machines/`: three valid check/HIR fixtures for bare `yield;` in a machine state body, nested block, and `if` branch, plus seven invalid parse fixtures covering ordinary-function `yield;` (`CON0300`), nested ordinary-block `yield;` (`CON0300`), top-level parser rejection, `yield 1;` (`CON0302`), `yield return 1;` (`CON0303`), expression-position `yield` (`CON0301`), and missing semicolon. The compiler keeps token/parser/AST/HIR representation and source-span preservation for a no-payload yield statement. MIR/backend lowering, state preservation behavior, scheduler/async/generator semantics, `yield return`, yield values, `suspend`, continuation yield, and DragonGod hooks remain out of scope.
