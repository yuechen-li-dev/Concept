...
Validation on the qualified host: `go vet ./...`, both `zig build test` roots, direct Go BurnIn (`go test ./internal/concept -run R7d3 -count=1`), Standard build/test, DragonGod build/test, TinyXML2 check/test, Clang and GCC strict-C11 aggregate round trips, and the ABI/layout corpus passed. `oct make BurnIn --file Make.oct` could not start because `oct` is not on PATH; the exact Go target from `Make.oct` passed. The full Go suite was rerun after isolating the ABI determinism test.
