...
| async interface method | interface callable pressure | absent | ordinary interface method normalized to an `Async<T>` result | EVT1-new composition / implemented R5h | No | R5h struct/class/static corpus | no async implements mechanism |
| async witness entry | dynamic call slots | absent | existing witness entry returns concrete inline `Async<T>` | Redesign / implemented R5h | No | R5h MIR/C/determinism evidence | one `(interface, concrete)` witness, not per invocation |
| dyn async dispatch | bounded interface dispatch | absent | witness-indirect constructor once, then concrete generated machine | EVT1-new composition / implemented R5h | No | R5h immediate-await/manual-drive native corpus | no virtual Step or async vtable |
| async interface receiver lifetime | lifetime pressure only | borrowed context pressure | returned operation retains concrete receiver provenance | Merge / implemented R5h | No | R5h scoped/local/const cases | no provenance laundering through dyn |
| owning async dyn/callback | legacy erased-storage pressure | absent | deferred pending explicit capture and storage policy | Deferred | Yes | R5h direction only | no hidden heap, generalized closure, or task object |
...
| yield | explicit re-entry suspension without lifted locals | absent | bare state-only Step end; same-state restart from beginning | Keep PoC3 semantic core / implemented R5d | Translation | phase19 plus R5d native/MIR/Planner | no saved PC, frame, scheduler, or result write |
| iterator | explicit state-machine direction only | absent | `GetIterator`/`MoveNext`/`Current` value protocol | EVT1-new / implemented R5d | No | R5d custom iterator native case | inline state; no hidden allocation |
| foreach | absent | absent | typed sugar over explicit iterator progression | EVT1-new / implemented R5d | No | R5d array/ndarray/Span/custom corpus | transient inside a yielded state |
| generator yield value | deferred | absent | keep separate from bare machine yield | Deferred | Yes | phase19 rejection evidence | would require item production and continuation semantics |
