...
## DG6 status: Decision subsystem v0

DG6 is complete as an integer deterministic `DragonGod.Kernel.Decision` subsystem. The v0 surface contains `DecisionSlot`, `DecisionPolicy`, `DecisionMemory`, `UtilityOption`, `decisionPolicyDefault`, `decisionMemoryEmpty`, `utilityOption`, `decisionAbsDiff`, `decisionBestOption4`, and `decisionChoose4`.

`decisionChoose4` evaluates four fixed options in parameter order, ignores disabled options, chooses the highest enabled integer score, preserves source-order ties through strict score comparison, returns an explicit fallback when no option is enabled, records the first current option immediately, refreshes score when the best option is already current, blocks switching inside `minCommitTicks`, blocks switching unless the best score exceeds `currentScore + hysteresisDelta`, keeps current when the absolute score difference is within `tieEpsilon`, and otherwise switches current option and records `lastSwitchTick`.

DG6 composes with DG3 Memory by reading integer scores from `memoryRead`, composes with DG5 Mind by feeding the chosen `StateId` into `AutomataSignal::Goto`, and reinforces the doctrine that native `transition decide` is still the correct tool for local stateless machine transitions. Deferred: floating scores, probabilistic/random selection, dynamic option lists, generic consideration functions, changed-key filtering, dynamic automaton graphs, transition scanning, actuation/events/trace/replay/persistence/parallel execution, scheduler/async, and DragonGod compiler hooks.

## Phase 20 / DG7 Actuation subsystem v0 status

DG7 is complete as an explicit kernel actuation host scaffold, not as an external command executor. `DragonGod.Kernel.Actuation` defines `ActuationStatus`, `ActuationDecision`, `ActuationDispatchResult`, fixed-slot `ActuationSlot`, and fixed-capacity `ActuatorHost`. Allowed dispatch allocates the next integer `ActuationId`, occupies the first empty slot, records `Pending`, and returns an accepted result. Denied dispatch allocates an id for deterministic accounting but does not occupy a slot; it returns a completed, not-ok denied result with the deny `Reason`. `actuatorComplete` and `actuatorFail` mark matching occupied slots terminal, while `actuatorStatus` reports `Empty` for unknown ids.
...
## Phase 20 / DG9 Trace subsystem v0 update

DG9 adds `DragonGod.Kernel.Trace` as an explicit kernel trace observation surface, not a logging framework or hidden instrumentation runtime. The v0 model defines `TraceEventKind` (`Empty`, `Enter`, `Exit`, `Transition`, `Yield`, `Signal`), `TraceEvent`, and a fixed four-slot `TraceRecorder`. Helpers cover `traceEmptyEvent`, `traceRecorderEmpty`, `traceCount`, `traceAt`, `traceEnter`, `traceExit`, `traceTransition`, `traceYield`, `traceSignal`, `traceEventKindCode`, and `traceSignalReasonCode`.

Trace v0 appends deterministically in slot order, increments count, panics with `DragonGod.Trace capacity exceeded` on overflow, and panics with `DragonGod.Trace index out of range` for invalid indexed reads. `AutomataSignal` is stored directly in `TraceEvent`, and signal reason interpretation uses `match`. `TraceSink` interface shape and borrowed `dyn TraceSink&` are smoke-tested, but `TraceRecorder` remains the supported v0 implementation because DG9 intentionally avoids sink registries, async sinks, file/stdout logging, telemetry protocols, persistence, replay, parallel merge, event bus integration, and compiler-injected hooks.
