...
P22-M2 is implemented. Option matching uses unqualified `Some(value)` and `None` arms, with `_` accepted as the existing wildcard/default arm. The checker rejects incomplete Option matches (`CON0502` for missing `None`, `CON0513` for missing `Some`), duplicate Option arms (`CON0514`), and invalid Option match arms (`CON0515`). `Some(value)` binds `value` as the Option element type, and lowering inspects the generated tag (`1` for Some, `0` for None) without reading payload data in the `None` arm. The helper surface now includes `optionIsNone(value)` and `optionUnwrapOr(value, fallback)` alongside the preserved `optionIsSome` and `optionOr`; `optionUnwrapOr` is the clearer canonical name, while `optionOr` remains accepted for M1 compatibility. `optionExpect` is deferred because Phase 22 has not added strings/formatting beyond the existing panic reason surface. Result, BufferError, FixedBuffer try APIs, and DragonGod migration remain deferred.
