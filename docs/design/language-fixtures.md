...
Phase 19 fixtures live under `language/phase19-yielding-machines/`. P19-M4 through P19-M6 were completed together as the `P19-M4-M6: yield behavior and interaction fixture sweep`, raising the set to twenty-one fixtures: ten validation fixtures, nine run fixtures, and two backend C fixtures. The run fixtures cover `yield + Complete`, `yield + Result`, `yield + State`, branch-local yield, nested child ticking, child `State` readability after parent yield, repeated external `Step` polling, `transition match`, and `transition decide`. The backend fixtures prove direct `return;` lowering and absence of hidden allocation, scheduler, async, event, mailbox, blackboard, or DragonGod runtime markers. No new semantics were added beyond fixture hardening; diagnostics/runtime hardening, examples, and Phase 19 closeout remain.
