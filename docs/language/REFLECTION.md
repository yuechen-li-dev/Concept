...
Generated functions are inspectable with `concept generated <file> [symbol]` and attributable with `concept explain <file> --generated <symbol>`. The generator can expand source-ordered annotated fields with `Fields<T>(attribute)`. This bounded API does not expose `TypeInfo` as an arbitrary runtime or comptime value, and it does not yet derive enum matches or composite traversals. See [checked generated declarations](../design/EVT1-GENERATED-DECLARATIONS.md).
