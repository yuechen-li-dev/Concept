...
test "keywords tokenize as keyword kinds" {
    const source_text = "module import extern export struct enum concept interface impl marker unsafe mut const return try move comptime alloc noalloc static_assert must_use discard match decide when if else while for machine state transition yield run true false";
    const expected = [_]TokenKind{ .module, .import, .@"extern", .@"export", .@"struct", .@"enum", .concept, .interface, .impl, .marker, .unsafe, .mut, .@"const", .@"return", .@"try", .move, .@"comptime", .alloc, .noalloc, .static_assert, .must_use, .discard, .match, .decide, .when, .@"if", .@"else", .@"while", .@"for", .machine, .state, .transition, .yield, .run, .true, .false, .eof };
...
test "allocation effect keywords tokenize without consuming nearby identifiers" {
    const expected = [_]TokenKind{ .alloc, .noalloc, .identifier, .identifier, .identifier, .identifier, .eof };
    try expectLexedKinds("alloc noalloc allocate noallocator allocation noalloc_mode", &expected);
}
